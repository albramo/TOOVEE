...
class StickyAddToCartComponent extends Component {
  ...
  #handleScroll = () => {
    ...
    const isAboveViewport = rect.bottom <= 60;
    const isScrolled = (window.scrollY || document.documentElement.scrollTop || 0) > 80;

    if (isAboveViewport && isScrolled) {
      ...
      this.#buyButtonsIntersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const isScrolledPast = rect.bottom <= 60 || rect.top < 0;
            if (isScrolledPast && window.scrollY > 80) {
              if (!this.#isChatActive()) {
                this.#showStickyBar();
              } else {
                this.#hideStickyBar();
  ...
  #showStickyBar() {
    ...
    stickyBar.style.setProperty('transform', 'translateX(-50%) translateY(0%)', 'important');
    stickyBar.style.setProperty('opacity', '1', 'important');
    stickyBar.style.setProperty('pointer-events', 'auto', 'important');
    stickyBar.style.setProperty('display', 'flex', 'important');
  }

  #hideStickyBar() {
    // Kept visible per user request
    this.#showStickyBar();
  }
